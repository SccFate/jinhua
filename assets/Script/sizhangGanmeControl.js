cc.Class({
		...
		Global.socket.onmessage = function (evt) {
			...
			if (evt.data.length > 0) {
				...
				switch (info.state) {
					...
					case 4:
						...
						if (info.Roler == "1") {
							...
							self.onNetMessage("got zhuang", JSON.stringify({
								deskNum: parseInt(info.BankerPos)
							}));
							for (var k in self.playInfoArray) {
								if (k.indexOf("play") >= 0) {
									if (self.deskNum != self.playInfoArray[k].deskNum) {
					...
					case 5:
						if (parseInt(info.Pos) == self.deskNum) {
							if (info.RoadNum == "0") {
								...
								self.endSchedule();
								self.xiazhuBeishuButtonNode.getChildByName("coin").getComponent(cc.Label).string = "0";
								self.xiazhuBeishuButtonNode.getChildByName("dao").getComponent(cc.Label).string = "第" + info.RoadNum + "道下注";
								self.startSchedule();
							}
						}
						self.onNetMessage("got roleXiazhu", JSON.stringify({
					...
					case 18:
						self.endSchedule();
						if (self.playInfoArray["play3"]) {
							if (self.playInfoArray["play3"].deskNum == parseInt(info.Pos)) {
								self.onNetMessage("got rolepoker", JSON.stringify({

									deskNum: parseInt(info.Pos),
									pokerArray: info.Card2.split("|").concat(info.Card1.split("|")),
									Num1: info.Num1,
									SanDian1: info.SanDian1,
									CardPoint1: info.CardPoint1,
									Num2: info.Num2,
									SanDian2: info.SanDian2,
									CardPoint2: info.CardPoint2
								}));
							}
							else {
								self.onNetMessage("got rolepoker", JSON.stringify({
									...
									deskNum: parseInt(info.Pos),
									pokerArray: info.Card1.split("|").concat(info.Card2.split("|")),
									Num1: info.Num1,
									SanDian1: info.SanDian1,
									CardPoint1: info.CardPoint1,
									Num2: info.Num2,
					...
					case 8:
						...
						if (info.Card.length > 0) {
							setTimeout(function () {
								self.playInfoArray["play1"].pokerArray = info.Card.split("|");
	...
	startListen: function () {
		...
		self.addMessage("got roleXiazhu", function (msg) {
			...
			if (info.beishu != "") {
				if (role.node.getChildByName("xiazhu").getComponent(cc.Label).string.length > 0) {
					role.node.getChildByName("xiazhu").getComponent(cc.Label).string = role.node.getChildByName("xiazhu").getComponent(cc.Label).string + "\n" + info.beishu;
				}
				else {
					role.node.getChildByName("xiazhu").getComponent(cc.Label).string = info.beishu;
				}
			}
		...
		self.addMessage("got rolepoker", function (msg) {
			...
			if (self.pokerNum != 2) {
				var pos1 = role.node.getChildByName("pokerGroup").getChildByName("pos1").position;
				for (var i = 1; i < 5; i++) {
					var poker = null;
					if (i < info.pokerArray.length + 1) {
						cc.log(info.pokerArray[i - 1]);
						if (info.pokerArray[i - 1].split("-")[1] == "1") {
							info.pokerArray[i - 1] = info.pokerArray[i - 1].split("-")[0] + "-14";
						}
						if (info.pokerArray[i - 1].split("-")[1] == "16") {
							...
							poker.getChildByName("num").getComponent(cc.Sprite).spriteFrame = self.pokerArray[info.pokerArray[i - 1]].num;
							poker.getChildByName("smallHuase").getComponent(cc.Sprite).spriteFrame = self.pokerArray[info.pokerArray[i - 1]].smallHuase;
							poker.getChildByName("bigHuase").getComponent(cc.Sprite).spriteFrame = self.pokerArray[info.pokerArray[i - 1]].bigHuase;
						}
					}
					else {
						poker = cc.instantiate(self.pokerBackPre);
					...
					var d = 255;
					if (typeof self.playInfoArray["play3"] != "undefined") {
						if (role.deskNum == self.playInfoArray["play3"].deskNum) {
							d = -255;
						}
					}
					if (i >= 3) {
						poker.setPosition(role.node.getChildByName("pokerGroup").getChildByName("poker" + i).position);
						poker.runAction(cc.moveTo(0.5, role.node.getChildByName("pokerGroup").getChildByName("pos" + (i - 1))));
					}
					else {
						poker.runAction(cc.moveTo(0.5, cc.p(role.node.getChildByName("pokerGroup").getChildByName("pos" + (i + 1)).x, role.node.getChildByName("pokerGroup").getChildByName("pos" + (i + 1)).y + d)));
					}
				}
			}
			setTimeout(function () {
				if (info.Num1) {
					if (info.Num1.length > 0) {
						cc.log("牌型" + info.Num1);
						role.node.getChildByName("type1").active = true;
						if (info.Num1 == "79") {
							if (info.CardPoint != "0") {
								if (parseInt(info.SanDian1) > 6) {
									cc.log("---------------" + self.typeList["t" + info.SanDian1 + "-" + info.CardPoint1]);
									role.node.getChildByName("type1").getComponent(cc.Label).string = self.typeList["t" + info.SanDian1 + "-" + info.CardPoint1];
									self.audioControl.playSFX(info.SanDian1 + "-" + info.CardPoint1);
								}
								else {
									cc.log("no ---" + self.typeList["t" + info.CardPoint1]);
									role.node.getChildByName("type1").getComponent(cc.Label).string = self.typeList["t" + info.CardPoint1];
									self.audioControl.playSFX(info.CardPoint1);
								}
							}
							else {
								role.node.getChildByName("type1").getComponent(cc.Label).string = "毕十";
								self.audioControl.playSFX(info.CardPoint1);
							}
						}
						else {
							role.node.getChildByName("type1").getComponent(cc.Label).string = self.typeList["t" + info.Num1];
							self.audioControl.playSFX(info.Num1);
			...
			setTimeout(function () {
				if (info.Num2) {
					if (info.Num2.length > 0) {
						cc.log("牌型" + info.Num2);
						role.node.getChildByName("type2").active = true;
						if (info.Num2 == "79") {
							if (info.CardPoint2 != "0") {
								if (parseInt(info.SanDian2) > 6) {
									cc.log("---------------" + self.typeList["t" + info.SanDian2 + "-" + info.CardPoint2]);
									role.node.getChildByName("type2").getComponent(cc.Label).string = self.typeList["t" + info.SanDian2 + "-" + info.CardPoint2];
									self.audioControl.playSFX(info.SanDian2 + "-" + info.CardPoint2);
								}
								else {
									cc.log("no ---" + self.typeList["t" + info.CardPoint2]);
									role.node.getChildByName("type2").getComponent(cc.Label).string = self.typeList["t" + info.CardPoint2];
									self.audioControl.playSFX(info.CardPoint2);
								}
							}
							else {
								role.node.getChildByName("type2").getComponent(cc.Label).string = "毕十";
								self.audioControl.playSFX(info.CardPoint2);
							}
						}
						else {
							role.node.getChildByName("type2").getComponent(cc.Label).string = self.typeList["t" + info.Num2];
							self.audioControl.playSFX(info.Num2);
		...
		self.addMessage("got poker", function (msg) {
			...
			for (var i = 1; i < 5; i++) {
				...
				self.playInfoArray["play1"].pokerArray[i - 1] = {
					poker: info.pokerArray[i - 1],
					isChoosed: false,
					node: poker
				};
			}
		...
		self.addMessage("fapai", function (msg) {
			...
			for (var k = 1; k < 5; k++) {
				...
				for (var i = 1; i < 5; i++) {
					...
					poker.runAction(cc.sequence(cc.moveTo(0.1 * p, poker.position), cc.moveTo(0.2, role.getChildByName("pokerGroup").getChildByName("pos" + i))));
					p++;
				}
			}
		...
		self.addMessage("got fapai", function (msg) {
			var info = JSON.parse(msg);
			for (var j = 1; j < 5; j++) {
				var role = self.playNode.getChildByName("play" + j);
				for (var i = 1; i < 5; i++) {
					var poker = cc.instantiate(self.pokerBackPre);
					poker.scaleX = 1.7;
					poker.scaleY = 1.9;
					poker.name = "poker" + i;
					role.getChildByName("pokerGroup").addChild(poker);
					poker.setPosition(role.getChildByName("pokerGroup").getChildByName("pos" + i).convertToNodeSpace(cc.p(405, 640)));
					poker.runAction(cc.moveTo(0.5, role.getChildByName("pokerGroup").getChildByName("pos" + i)));
				}
			}
		});
	...
	resetRoomData: function () {
		this.xiazhuBeishuButtonNode.getChildByName("coin").getComponent(cc.Label).string = "0";
		this.xiazhuBeishuButtonNode.getChildByName("dao").getComponent(cc.Label).string = "第" + 1 + "道下注";
		this.heartTime = 0;
		for (var k = 1; k < 5; k++) {
			...
			role.getChildByName("type1").getComponent(cc.Label).string = "";
			role.getChildByName("type2").getComponent(cc.Label).string = "";
		}
	},
	clearInfo: function () {
	...
	shareButtonClick: function () {
		...
		jsb.reflection.callStaticMethod("org/cocos2dx/javascript/AppActivity", "shareWebPage", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V", 0 + "", "至尊宝 牌九【房间号:" + this.roomNum + "】", this.roomData, "http://www.baidu.com");
	...
	startTouchControl: function () {
		var self = this;
		var pokerGroup = cc.find("Canvas/mainNode/UI/Play/Node6/play1/pokerGroup");
		for (var j = 0; j < self.playInfoArray["play1"].pokerArray.length; j++) {
			self.playInfoArray["play1"].pokerArray[j].isChoosed = false;
		}
		this.adjustMyPokerArrayPosition();
		pokerGroup.on(cc.Node.EventType.TOUCH_START, function (event) {
			var touches = event.getTouches();
			var touch = pokerGroup.convertToNodeSpace(touches[0].getLocation());
			var width = 730;
			var height = 500;
			self.isOnPokerTouch = true;
			self.pokerClickX = touch.x;
			self.beginIndex = self.getChoosePokerIndex(touch.x);
			if (self.beginIndex < self.playInfoArray["play1"].pokerArray.length) {
				self.playInfoArray["play1"].pokerArray[self.beginIndex].node.getChildByName("opacity").active = true;
			}
			var arr = [];
			for (var j = 0; j < self.playInfoArray["play1"].pokerArray.length; j++) {
				arr.push(self.playInfoArray["play1"].pokerArray[j].isChoosed);
			}
			self.pokerStateArr = arr.concat([]);

		}, self);
		pokerGroup.on(cc.Node.EventType.TOUCH_MOVE, function (event) {
			...
			if (self.isOnPokerTouch) {
				if (touch.x > width || touch.x < 0 || touch.y > height || touch.y < 0) {

				}
				else {
					var dx = touch.x - self.pokerClickX;
					if (Math.abs(dx) > 20) {
						var moveX = touch.x;
						for (var j = 0; j < self.playInfoArray["play1"].pokerArray.length; j++) {
							self.playInfoArray["play1"].pokerArray[j].isChoosed = self.pokerStateArr[j];
						}
						if (moveX > self.pokerClickX) {
							for (var k = self.pokerClickX; k < moveX; k++) {
								if (self.getChoosePokerIndex(k) < self.playInfoArray["play1"].pokerArray.length) {
									self.playInfoArray["play1"].pokerArray[self.getChoosePokerIndex(k)].isChoosed = !self.pokerStateArr[self.getChoosePokerIndex(k)];
									self.playInfoArray["play1"].pokerArray[self.getChoosePokerIndex(k)].node.getChildByName("opacity").active = true;
								}
							}
						} else {
							for (var x = moveX; x < self.pokerClickX; x++) {
								if (self.getChoosePokerIndex(x) < self.playInfoArray["play1"].pokerArray.length) {
									self.playInfoArray["play1"].pokerArray[self.getChoosePokerIndex(x)].isChoosed = !self.pokerStateArr[self.getChoosePokerIndex(x)];
									self.playInfoArray["play1"].pokerArray[self.getChoosePokerIndex(x)].node.getChildByName("opacity").active = true;
								}
							}
						}
						self.isChooseMove = true;

					}
				}
			}
		}, self);
		pokerGroup.on(cc.Node.EventType.TOUCH_END, function (event) {
			var touches = event.getTouches();
			var touch = pokerGroup.convertToNodeSpace(touches[0].getLocation());
			var width = 730;
			var height = 500;
			for (var j = 0; j < self.playInfoArray["play1"].pokerArray.length; j++) {
				self.playInfoArray["play1"].pokerArray[j].node.getChildByName("opacity").active = false;
			}

			if (self.isOnPokerTouch) {
				if (self.beginIndex == self.getChoosePokerIndex(touch.x)) {
					if (self.getChoosePoker().allNum < 2 || self.pokerStateArr[self.getChoosePokerIndex(touch.x)]) {



						if (self.getChoosePokerIndex(touch.x) < self.playInfoArray["play1"].pokerArray.length) {
							self.playInfoArray["play1"].pokerArray[self.getChoosePokerIndex(touch.x)].isChoosed = !self.pokerStateArr[self.getChoosePokerIndex(touch.x)];
						}
					}

				}
				self.adjustMyPokerArrayPosition();
				self.audioControl.playSFX("pclick");
			}

			self.isOnPokerTouch = false;
		}, self);
		pokerGroup.on(cc.Node.EventType.TOUCH_CANCEL, function (event) {
	...
	adjustMyPokerArrayPosition: function () {
		var num = 0;
		for (var i = 0; i < this.playInfoArray["play1"].pokerArray.length; i++) {
			if (this.playInfoArray["play1"].pokerArray[i].isChoosed) {
				num++;
				if (num <= 2) {
					this.playInfoArray["play1"].pokerArray[i].node.y = 100;

				}
				else {
					this.playInfoArray["play1"].pokerArray[i].isChoosed = false;
					this.playInfoArray["play1"].pokerArray[i].node.y = 0;
				}
	...
	getChoosePoker: function () {
		...
		for (var i = 0; i < this.playInfoArray["play1"].pokerArray.length; i++) {
			if (this.playInfoArray["play1"].pokerArray[i].isChoosed) {
				allNum++;
				p.push(this.playInfoArray["play1"].pokerArray[i].poker);
			}
			else {
				q.push(this.playInfoArray["play1"].pokerArray[i].poker);
			}
		}
		return {
			head: p[0] + "|" + p[1],
	...
	startFlyClip: function (info) {
		cc.log("fly fly" + info);
		var self = this;
		var zhuangRole = self.zhuangRole;
		setTimeout(function () {
			for (var i in info.resultArr) {
				...
				if (info.resultArr[i].result < 0) {
					var dy = zhuangRole.node.y - role.node.y;
					var dx = zhuangRole.node.x - role.node.x;
					var dr2 = dy * dy + dx * dx;
			...
			setTimeout(function () {
				for (var i in info.resultArr) {
					...
					if (info.resultArr[i].result > 0) {
						...
						self.audioControl.playSFX("coin");
						var flyClipNode = cc.instantiate(self.flyClipPre);
						zhuangRole.node.parent.addChild(flyClipNode);
						flyClipNode.setPosition(zhuangRole.node.position);
						var ps = flyClipNode.getComponent(cc.ParticleSystem);
						//  ps.duration=t;
						ps.life = t;
						ps.angle = angle;
