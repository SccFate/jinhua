...
cc.Class({
				...
				switch (info.state) {
					case 2:
						...
						if (info.Roler == 1) {
							...
							self.onNetMessage("got roleZhanqi", JSON.stringify({ Pos: self.deskNum + "", NickName: "你" }));
							for (var i = 1; i < 5; i++) {
					...
					case 19:
						...
						//self.showToast(JSON.stringify(info));
						if (info.type == "voice") {
							role.node.getChildByName("msgNode").active = true;
	...
	startFapai: function (msg) {
		var self = this;
		var p = 1;
		for (var k = 1; k < 5; k++) {
			var role = self.playNode.getChildByName("play" + k);
			...
			for (var i = 2; i < 4; i++) {
				(
					function (x) {
						...
						poker.name = "poker" + x;
						role.getChildByName("pokerGroup").addChild(poker);
						poker.setPosition(role.getChildByName("pokerGroup").convertToNodeSpace(cc.p(360, 640)));
						setTimeout(function () {
							poker.runAction(cc.moveTo(0.5, role.getChildByName("pokerGroup").getChildByName("pos" + x).position));
